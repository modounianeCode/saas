...
export default function Home() {
  ...
  const [form, setForm] = useState({ type: "expense" as "income" | "expense", amount: "", category: "Alimentation", occurred_on: new Date().toISOString().slice(0, 10) });
  ...
  async function addTransaction(event: FormEvent) {
    event.preventDefault(); if (!userId || !form.amount) return;
    const { error } = await client.from("transactions").insert({ user_id: userId, type: form.type, amount: Math.round(Number(form.amount)), label: form.category, category: form.category, occurred_on: form.occurred_on });
    if (error) return setNotice(error.message);
    setForm({ type: "expense", amount: "", category: "Alimentation", occurred_on: new Date().toISOString().slice(0, 10) }); load(userId);
  }
